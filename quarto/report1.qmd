...
if (params$adm_level %in% unique(df$ADM_LEVEL) & params$year %in% unique(df$YR)){
  df <- df %>% filter(ADM_LEVEL == params$adm_level & YR == params$year)
} else{
  stop("ERROR: either your adm_level given or your year does not exist LMAO")
}
```
...
```{r}
pyr <- plot_pyr(df,"ADM0",params$adm_name)
pyr
```

## Choropleth Map: `r params$adm_name` `r params$year`
```{r}
plot_chlor(tanzania_adm0, df,params$adm_name,params$year,paste0("ADM",params$adm_level))
```
